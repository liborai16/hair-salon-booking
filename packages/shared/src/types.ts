...
export interface BookingCustomer {
  ...
  /**
   * Magic-link cancel token — a bearer capability.
   *
   * - URL-safe base64, generated by the `createBooking` Cloud Function via
   *   `crypto.randomBytes(32).toString("base64url")` → 43 chars, 256 bits.
   * - Validated by `manageBookingByToken` via an INDEXED EQUALITY LOOKUP
   *   (`bookingCustomers.where('cancelToken','==',token)`), NOT a constant-time
   *   compare. There is no secret-vs-input comparison in app code to
   *   time-attack — the match happens in Firestore's index — so the 256-bit
   *   entropy is what makes guessing infeasible. See D-017.
   * - Stored plaintext: the collection is STAFF-ONLY readable, so the token
   *   itself acts as the capability — no extra hashing needed. Threat model:
   *   external clients guessing tokens; 256-bit entropy defeats brute force.
   */
  cancelToken: string;
}
